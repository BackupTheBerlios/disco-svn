...
return true;}
mozile.notifyChange=function(target){console.debug("firing change event");var event=document.createEvent("Events");event.initEvent("change",true,true);console.debug("dispatchEvent");target.dispatchEvent(event);}
...
return img;}
mozile.gui.htmlToolbar.create=function(){if(this.toolbar)return;var href=[mozile.root,"src","gui","htmlToolbar.css"].join(mozile.filesep);mozile.dom.addStyleSheet(href,"text/css");this.toolbar=mozile.gui.htmlToolbar.createElement("div");this.toolbar.setAttribute("id","mozileToolbar");mozile.dom.setClass(this.toolbar,"mozileGUI");mozile.protectElement(this.toolbar);var body=mozile.dom.getBody();body.appendChild(this.toolbar);if(mozile.browser.isIE){mozile.dom.setStyle(this.toolbar,"background-color",this.ieColor);}
